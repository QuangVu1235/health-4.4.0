...
class _HealthAppState extends State<HealthApp> {
  ...
  Future fetchData() async {
    ...
    final yesterday = now.subtract(Duration(days: 765));
    ...
    if (requested) {
      ...
      _healthDataList.forEach((x) => {
            if (DateFormat('yyyy-MM-dd').format(x.dateFrom) == DateFormat('yyyy-MM-dd').format(x.dateTo))
              {
                if (map.containsKey(DateFormat('yyyy-MM-dd').format(x.dateFrom)))
                  {map[DateFormat('yyyy-MM-dd').format(x.dateFrom)] += (x.value as NumericHealthValue).numericValue}
                else
                  {
                    map[DateFormat('yyyy-MM-dd').format(x.dateFrom)] = (x.value as NumericHealthValue).numericValue,
                  }
  ...
  Future fetchStepData() async {
    ...
    var instDay = now.difference(lastYear).inDays;
    Map<DateTime, int> stepsList = {};
    for(var i = 0 ; i < instDay; i++){
      var date = lastYear.add(Duration(days: i));
      var date2 = lastYear.add(Duration(days: i+1));
      steps = await health.getTotalStepsInInterval(date, date2);
      if(steps != null) stepsList[date] = steps;
      print('Total number of steps: $steps');
    }
    stepsList.removeWhere((key, value) => value == 0);
    bool requested = await health.requestAuthorization([HealthDataType.STEPS]);

    if (requested) {
      try {
        steps = await health.getTotalStepsInInterval(midnight.subtract(Duration(days: 1)), midnight);
      } catch (error) {
  ...
  Widget _stepsFetched() {
    return ListView.builder(
        itemCount: _nofSteps.length,
        itemBuilder: (_, index) {
          return ListTile(
            title: Text("Steps: ${_nofSteps.values.elementAt(index)}"),
            subtitle: Text('Date: ${_nofSteps.keys.elementAt(index)}'),
          );
        });
  }
